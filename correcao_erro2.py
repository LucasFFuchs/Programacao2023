#Solução linha 55
#Acho que o erro do chatgpt foi escrever um código que roda em apenas algumas versões do python/pygame antes de fazer um que rode para todos. Talvez isso tenha acontecido pois ele quer poupar espaço.
...
while executando:
    ...
    #Solução:
    #Definir separademente cada um deles

    minutos = int(tempo_atual // 60)
    segundos = int(tempo_atual % 60)
    milissegundos = int((tempo_atual % 1) * 1000)

    tempo_string = f"{minutos:02d}:{segundos:02d}.{milissegundos:03d}"
