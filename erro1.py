#O problema está na função (handle_button_click) na linha 81
...
def create_2player_gui():
    ...
    def handle_button_click(player, user_choice):
        if player == 1:
            player1_choice = user_choice#variável definida apenas nessa condição
            player_label.config(text="Jogador 2, faça sua escolha.")
        else:
            play_2player_game(player1_choice, user_choice)#Chama a função (player1_choice) com uma variável definida localmente(linha 83)
            window.destroy()
